...
Notice that the only way to interact with a iterator is to make it produce a
...
Now our `wood_basket` is an iterable object (an iterable but not an iterator)
and therefore it can be used inside the `for` statement.
...
After this article I'd strongly encourage reading [Revisiting the Mechanism
Behind the `for` Statement] [for_loop_mechanism] because this article explains
how the `for` statement uses the concepts you've learned here to iterate over
iterable objects.
